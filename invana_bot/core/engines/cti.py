...
class InvanaBotRunnerEngine(RunnerEngineBase):
    ...
    def crawl(self):
        ...
        if len(errors) == 0:
            ...
            initial_spider_config = get_crawler_from_list(
                spider_id=self.manifest['init_crawler']['spider_id'],
                spiders=self.manifest['spiders']
            )
            initial_spider_config['start_urls'] = self.manifest['init_crawler']['start_urls']
            crawler_runner = InvanaBotSingleCrawlerRunnerEngine(
                job_id=self.job_id,
                spider_cls=self.spider_cls,
                spider_config=initial_spider_config,
                manifest=self.manifest,
            )
            job = crawler_runner.run()
